...
function lex_comment_multiLine(bytes, count, pos, trackedSyntaxList) {
    ...
    if (seenLineFeed) {
        trackedSyntaxList.insert(trackedSyntaxList.count_abstract, get_TrackedSyntaxKind_Comment(), start, length);
    }
...
function JS_line_lex_newVersion(div, beltIndexOfDiv, indexLine, lineStart) {
    ...
    // ...tracked syntax is sorted...
    // ...you ought to be able to re-use the trackedSyntax_I you left on...
    // ...rather than refinding it each invocation by passing it as a parameter.
    let trackedSyntax_I;
